...
def get_stats(vocab):
    pairs = collections.defaultdict(int)  # initialize an empty dictionary

    for word, freq in vocab.items():
        symbols = word.split()  # character
        for i in range(len(symbols) - 1):
            pairs[symbols[i], symbols[i + 1]] += freq  # 字符两两组合, 计算频率, 包括end字符'</w>'
    return pairs
...
def merge_vocab(pair, v_in):
    ...
    # print('bigram_pattern: ', bigram_pattern)   # 对pair加转义字符 pair: ('e', 's'), bigram_pattern: 'e\\ s'
    # print ''.join(pair)    # es
    p = re.compile(r'(?<!\S)' + bigram_pattern + r'(?!\S)')

    for word in v_in:
        w_out = p.sub(''.join(pair), word)  # 替换 e s --> es
...
for i in range(num_merges):
    ...
    best = max(pairs, key=pairs.get)   # 找到频率最高的pair
    print best
    if pairs[best] < 2:
